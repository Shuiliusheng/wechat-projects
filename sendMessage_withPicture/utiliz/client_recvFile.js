...
//获取消息中的信息，进行下一步解析
function getMessage(unit8Arr, ip, port) {
  ...
  if (type == "fileinfo") {
    ...
    back.info = getImageInfo(unit8Arr, ip, port);
    return back;
  }
...
//如果消息是图像信息，则处理
function getImageInfo(unit8Arr, ip, port) {
  ...
  var temp = {};
  temp.date = datas[1];
  temp.username = datas[2];
  temp.filename = datas[3];
  temp.size = datas[4];
  temp.recvSize = 0;
  temp.reqPlace = 0;
  temp.src_ip = ip;
